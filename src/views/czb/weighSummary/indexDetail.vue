        ...
        <div class="head-container">
          ...
          >
            <el-button
              ...
              size="mini"
              icon="el-icon-search"
              :disabled="collectType === 5"
              @click="$refs.warehousePicker.open()"
            />
          </el-input>

          <label class="el-form-item-label">磅台</label>
          <el-input v-model="query.poundName" :disabled="collectType === 6" clearable placeholder="磅台" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
...
import { collectDetail, print, createUpOrder } from '@/api/czb/weighSummary'
...
export default {
  ...
  methods: {
    ...
    /**
     * @param collectType 汇总类型 1按产品汇总,2按往来单位汇总,3按司磅员汇总,4按日期汇总,5按仓库汇总,6按磅台汇总
     * @param value 汇总条件值
     */
    open(collectType, value, billStatus) {
    ...
    initParams() {
      // 汇总类型 1按产品汇总,2按往来单位汇总,3按司磅员汇总,4按日期汇总,5按仓库汇总,6按磅台汇总
      switch (this.collectType) {
        ...
        case 2:
          // 按往来单位
          this.query.billType = this.value.billType
          // 单据类型 1：采购入库 2：销售出库
          if (this.value.billType === '1') {
        ...
        case 3:
          // 3按司磅员汇总
          this.query.operatorId = this.value.id
          this.query.operatorName = this.value.label
          break
        case 4:
          // 按日期
          this.query.weighDate = this.value.label
          break
        case 5:
          // 按仓库
        ...
        case 6:
          // 6按磅台汇总
          this.query.poundName = this.value.label
          break
        default:
          break
      }
